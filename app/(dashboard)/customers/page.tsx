...
function CustomersContent() {
  const { filters, applyFilters, clearFilters } = useDashboardFilters("/customers");

  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["customers", normalizeFilters(filters)],
    queryFn: () => apiGet<AnalyticsResponse>("/analytics/customers/revenue", normalizeFilters(filters)),
  ...
  return (
    <div>
      <SectionHeader title="Clienti">
        <CsvExport data={data?.table.rows || []} filename="customers.csv" />
        <PrintButton />
      </SectionHeader>
