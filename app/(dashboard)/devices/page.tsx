...
function DevicesContent() {
  const { filters, applyFilters, clearFilters } = useDashboardFilters("/devices");

  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["devices", normalizeFilters(filters)],
    queryFn: () => apiGet<AnalyticsResponse>("/analytics/devices/revenue", normalizeFilters(filters)),
  ...
  return (
    <div>
      <SectionHeader title="Dispositivi">
        <CsvExport data={data?.table.rows || []} filename="devices.csv" />
        <PrintButton />
      </SectionHeader>
          ...
          {data && (
            <>
              {data.chart.series.length > 0 && (
                <ChartWrapper title="Incasso per Dispositivo">
