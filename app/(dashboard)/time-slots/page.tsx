...
const tabs = [
  { value: "by-hour", label: "Per Ora", endpoint: "/analytics/time/by-hour" },
  { value: "by-bucket", label: "Per Fascia", endpoint: "/analytics/time/by-bucket" },
];

export default function TimeSlotsPage() {
  return (
    <Suspense fallback={<TimeSlotsFallback />}> 
      <TimeSlotsContent />
    </Suspense>
  );
}
...
function TimeSlotsContent() {
  const [activeTab, setActiveTab] = useState("by-hour");
  const { filters, applyFilters, clearFilters } = useDashboardFilters("/time-slots");

  const currentEndpoint = tabs.find((t) => t.value === activeTab)?.endpoint || tabs[0].endpoint;

  const { data, isLoading, error, refetch } = useQuery({
    queryKey: ["time-slots", activeTab, normalizeFilters(filters)],
  ...
  return (
    <div>
      <SectionHeader title="Fasce Orarie">
        <CsvExport data={data?.table.rows || []} filename={`time-slots-${activeTab}.csv`} />
        <PrintButton />
      </SectionHeader>
